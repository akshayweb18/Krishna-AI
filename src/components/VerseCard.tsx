...
export default function VerseCard({ verse }: { verse: Shloka }) {
  ...
  return (
    <motion.article 
      ...
      className={`relative space-y-8 group p-8 md:p-12 rounded-[40px] transition-all duration-700 shadow-2xl border ${
        isBookmarked 
    ...
    >
      ...
      <div className={`absolute -top-4 -left-4 w-14 h-14 flex items-center justify-center rounded-2xl border font-serif text-lg shadow-[0_10px_30px_rgba(0,0,0,0.3)] z-10 group-hover:scale-110 transition-transform duration-500 ${
        isBookmarked ? 'bg-[#D4AF37] text-black border-[#D4AF37]' : 'bg-[#0A0E1A] text-[#D4AF37] border-[#D4AF37]/40'
      }`}>
        {verseId}
      </div>
      ...
      <button 
        ...
        className={`absolute -top-4 -right-4 w-12 h-12 flex items-center justify-center rounded-xl border transition-all duration-300 z-10 ${
          isBookmarked 
      ...
      <div className="text-center space-y-8 pt-4">
        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-[0.3em] shadow-inner transition-colors ${
          isBookmarked ? 'bg-[#D4AF37]/20 border-[#D4AF37]/30 text-[#D4AF37]' : 'bg-[#D4AF37]/10 border-[#D4AF37]/20 text-[#D4AF37]'
        }`}>
